...
const RULES: Array<{ type: string; pattern: RegExp }> = [
  { type: "dice-tower", pattern: /dice\s*tower/i },
  { type: "tuckbox", pattern: /tuck\s*box|card\s*box/i },
  { type: "card-holder", pattern: /card\s*(holder|stand|tray)/i },
  { type: "organizer", pattern: /organi[sz]er/i },
  { type: "insert", pattern: /\binsert\b/i },
  { type: "tokens", pattern: /\bcoins?\b|\btokens?\b|\bmeeples?\b/i },
