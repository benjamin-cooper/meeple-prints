...
export function ProductRow({
  ...
  return (
    <tr
      onClick={onClick}
      className="group cursor-pointer hover:bg-secondary/60 border-b border-border last:border-0 border-l-2 border-l-transparent hover:border-l-primary transition-colors"
    >
      ...
      <td className="py-1.5 pr-3 text-sm whitespace-nowrap">
        {product.rating ? (
          <span className="text-status-installed tracking-tight">{"★".repeat(product.rating)}</span>
        ) : (
